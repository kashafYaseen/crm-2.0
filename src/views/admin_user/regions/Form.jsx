...
const Form = observer((props) => {
  ...
  useEffect(() => {
    ...
    const fetch_data = async () => {
      try {
        const extractCountriesData = await countries_data('get', 'countries', null, null, authToken)
    ...
    onSubmit: async (values) => {
      ...
      if (formik.isValid) {
        if (region_data) {
          ...
          try {
            const extractedData = await regions_data('post', 'regions', values, null, authToken)
